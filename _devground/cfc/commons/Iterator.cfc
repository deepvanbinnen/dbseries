<cfcomponent output="true" hint="An unoptimized(!) object to loop over a given variable">
	<!--- Iterable properties --->
	...
	<cfset variables.itarray   = ArrayNew(1)><!--- If requested, iterable items as an Array --->
	...
	<cfset variables.maxiters    = -1><!--- maximum iterations to make, -1=(loop over entire collection) --->
	...
	<cffunction name="getFullType">
		<cfreturn variables.fulltype>
	</cffunction>
	...
	<cffunction name="_setIterableType">
		...
		<cfset variables.ittype   = LCASE(ListLast(variables.fulltype, "."))>
	</cffunction>
	...
	<cffunction name="_setLength">
		...
		<cfset this.length      = variables.length>
		<cfset variables.maxiters = variables.length>
	</cffunction>
	...
	<cffunction name="maxreached">
		<cfif getIndex() GT getLength()>
			<cfset variables.bmaxreached = true>
		</cfif>
		...
		<cfelse>
			<cfset variables.bmaxreached = (variables.maxiters neq -1 AND getIndex() gte variables.maxiters)>
		</cfif>
